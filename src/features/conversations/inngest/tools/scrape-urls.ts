...
export const createScrapeUrlsTool = () =>{
    return createTool({
        name: "scrapeUrls",
        description:"Scrape content from URLs to get documentation or reference material. Use this when the user provides URLs or reference external documentation. Returns markdown content from scraped pages.",
        parameters: z.object({
        ...
        handler: async(params,{step: toolStep}) =>{
            ...
            try {
                return await toolStep?.run("scrape-urls",async()=>{
                    ...
                    if(results.length===0) return "No content could be scraped from the provided URLs."
